...
| [`resultado`](resultado.fa) | Constructores de `Resultado<T,E>` y `Opcion<T>` | `Ok(valor)`, `Error(mensaje)`, `Some(valor)`, `None()` |
...
funcion main() {
    ...
    variable usuario = env.obtener("USER").Some("desconocido")
